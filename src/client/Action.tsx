...
export function MissionHeaderAction({ controller, openSidebar, sessionId, t }: HeaderProps): ReactNode {
  return (
    <button
      type="button"
      className="mc-action"
...
export function MissionSidebarAction({ controller, openSidebar, useSessions, wide, t }: SidebarProps): ReactNode {
  ...
  return (
    <button
      ...
      onClick={event => {
        openSidebar()
        controller.toggle(sessionId === undefined ? undefined : String(sessionId), event.currentTarget)
      }}
    >
      <span className="mc-action__mark" aria-hidden="true">◎</span>
