...
interface AgentTree { readonly agent: AgentView; readonly children: readonly AgentTree[] }
...
export function AgentGraph({ agents, rootId, selectedAgentId, select, t }: {
  ...
  const layout = tree<AgentTree>().nodeSize([112, 224])(hierarchy(root, item => item.children))
...
function buildTree(agents: readonly AgentView[], rootId: string): AgentTree | undefined {
  ...
  const visit = (agent: AgentView): AgentTree => ({
    agent,
